...
public class WaffleView extends JPanel implements MouseListener {
	...
	public void update() {
		...
		class CurrentState {
			public boolean isWaffel(int x, int y) {
				return (x != 2 && y != 3);
			}
		...
		for (int x = 0; x < 5; x++)// ////////engine.getWidth(); x++) // For
									// each wafle cell: draw the cell
		{
			xp = xMin + x * cellWidth;
			for (int y = 0; y < 5; y++)// //////engine.getHeight(); y++)
			{
				...
				if (cs.isWaffel(x, y))
					drawWaffle(xp, yp, drawable);
				else if (cs.isEaten(x, y))
					drawEatenWaffle(xp, yp, drawable);
				else if (cs.isPoison(x, y))
					drawPoison(xp, yp, drawable);
				else
	...
	public void resize(int width, int height) {
		...
		this.xMin = (int) (getWidth() * paddingWidth); // Position of the most
														// left waffle point
		this.yMin = (int) (getHeight() * paddingHeight);
		// /// this.cellWidth = (getWidth() - 2*xMin) / engine.getWidth(); //
		// Waffle cell size
	...
	/**
	 * ============================================================ Draww a
	 * waffle cell described by the most left and high pixel (xp, yp)
	 * ==============================================================
	 */
	private void drawWaffle(int xp, int yp, Graphics2D drawable) {
		...
		int dr = (borderInColor.getRed() - borderOutColor.getRed())
				/ nbrWaffleShades;
		int dg = (borderInColor.getGreen() - borderOutColor.getGreen())
				/ nbrWaffleShades;
		int db = (borderInColor.getBlue() - borderOutColor.getBlue())
				/ nbrWaffleShades;
		for (int shade = 0; shade < nbrWaffleShades; shade++) // Draw the border
																// shades
		{
			drawable.setPaint(c);
	...
	/**
	 * ============================================================ Draww an
	 * empty waffle cell described by the most left and high pixel (xp, yp)
	 * ==============================================================
	 */
	...
	/**
	 * ============================================================ Draww a
	 * poisoned waffle cell described by the most left and high pixel (xp, yp)
	 * ==============================================================
	 */
