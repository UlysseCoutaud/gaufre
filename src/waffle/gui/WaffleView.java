...
public class WaffleView extends JPanel implements MouseListener {
    ...
    // Constructeur:
    // --------------------------------------------
    public WaffleView(int width, int height, Engine engine) {
        this.engine = engine;
        try {
            this.poisonImage = ImageIO.read(new File(poisonImagePath));
        } catch (Exception e) {
            Logger.logGui("Error: Cannot find poison image file");
        }
    ...
    public void update() {
        ...
        class CurrentState {
            ...
            public boolean isWaffle(int x, int y) {
                return true;
            }
        ...
        for (int x = 0; x < 10; x++)// ////////engine.getWidth(); x++) // For
